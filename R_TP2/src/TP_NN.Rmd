...
installed. If `Anaconda` (or Miniconda) is not installed on your computer, download and install it. Then execute in the
terminal the command `conda env create -f keras.yaml` which will create a python 3.7 environment with tensorflow.


Then, run the following chunk that will load keras and the conda environment `keras` with the libraries required to run
Keras that you have installed above. You can check with the command `conda info --envs` run from a command line
interface that an environment named `keras` is indeed available on your computer. 

```{r}
library(keras)
use_condaenv(condaenv="keras")
```
